...
**Интервал автообновления** задаётся в каждой секции отдельно (`30m | 1h | 3h | 6h | 12h | 1d`, по умолчанию `1h`) и не зависит от порядка секций в конфиге: на каждое значение создаётся своё cron-задание, которое обновляет только секции с этим интервалом. `/usr/bin/netshift subscription_update` без аргумента обновляет все секции, с интервалом (`subscription_update 1d`) - только секции с ним.

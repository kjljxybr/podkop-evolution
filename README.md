...
Без группировки (`subscription_group_mode=off`) у каждой подписки с живыми узлами есть свой блок на дашборде и свой «Самый быстрый» (`⚡ <имя>`). Общий «Самый быстрый» по всем узлам остаётся выбором по умолчанию. Имя подписки берётся из `#фрагмента` ссылки, а без него из хоста. Фрагмент на сервер не отправляется и в ключ кеша не входит, поэтому переименование подписки не сбрасывает её кеш:
